...
describe("local-only privacy boundary", () => {
  ...
  it("keeps variation on the seeded hash rather than Math.random", async () => {
    for (const file of await sourceFiles()) {
      const source = stripComments(await readFile(file, "utf8"));
  ...
  it("loads uploaded marks from memory, never from a URL", async () => {
    const library = stripComments(
      await readFile(new URL("../src/engine/glyphLibrary.ts", import.meta.url), "utf8"),
    );
    expect(library).toContain("FileReader");
    expect(library).not.toMatch(/https?:\/\/(?!www\.w3\.org)/);
  });
});
