...
import { cellPixels, maxExportFrames, type ExportInk, type Settings } from "../types";
...
export async function* renderSequence(options: SequenceOptions): AsyncGenerator<RenderedFrame> {
  const { source, settings, ink, library, signal } = options;
  ...
  for (let index = 0; index < total; index += 1) {
    ...
    renderer.render({ settings, field, library, frame: index, ink, ramp });
